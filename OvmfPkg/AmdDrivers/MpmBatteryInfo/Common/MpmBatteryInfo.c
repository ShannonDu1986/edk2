...
)
{
  ...
  DEBUG ((DEBUG_INFO, "%as: enter\n", __FUNCTION__));
  Status = gBS->LocateProtocol (&gAmdMpmGetBatteryInfoProtocol, NULL, (VOID**)&AmdMpmGetBatteryInfo);
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_INFO, "ReadBatteryInfoCallback: failed to locate gAmdMpmGetBatteryInfoProtocol, Status = %r\n", Status));
    return;
  }

  Status = AmdMpmGetBatteryInfo->GetBatteryInfo(&BatteryInfo);
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_INFO, "ReadBatteryInfoCallback: Failed to get the battery information\n"));
    return;
  }
  DEBUG ((DEBUG_INFO, "ReadBatteryInfoCallback: BatteryInfo.BatteryStatus = 0x%x\n", BatteryInfo.BatteryStatus));
}

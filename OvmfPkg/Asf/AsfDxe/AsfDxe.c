...
)
{
  ...
  for (Index = 0; Index < Count; Index++) {
    if (mAsfProgressMsgList[Index].StatusCode == Value) {
      Status = AsfPushMessage((UINT8 *)&mAsfProgressMsgList[Index].Message,
                      mAsfProgressMsgList[Index].Message.Length + 2,
  ...
  for (Index = 0; Index < Count; Index++) {
    if (mAsfErrorMsgList[Index].StatusCode == Value) {
      Status = AsfPushMessage((UINT8 *)&mAsfErrorMsgList[Index].Message,
                      mAsfErrorMsgList[Index].Message.Length + 2,
